...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/05 08:37:13 by habouiba          #+#    #+#             */
/*   Updated: 2022/10/01 14:54:23 by habouiba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *handle_return(char **backup, char **line)
{
	...
	*backup = _ft_substr(*line, ft_strchr(*line, '\n') - *line + 1, -1);
	if (!**backup)
	{
		free(*backup);
		*backup = NULL;
	}
	tmp = _ft_substr(*line, 0, ft_strchr(*line, '\n') - *line + 1);
	free(*line);
	return (tmp);
}
